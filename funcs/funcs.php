...
function emailExiste($email) {
    ...
    $stmt = $con->prepare("SELECT id_usuario FROM usuarios WHERE correo = ? LIMIT 1");
...
function validaIdToken($id, $token) {
    ...
    $stmt = $con->prepare("SELECT activacion FROM usuarios WHERE id_usuario = ? AND token = ? LIMIT 1");
...
function activarUsuario($id) {
    ...
    $stmt = $con->prepare("UPDATE usuarios SET activacion=1 WHERE id_usuario = ?");
...
function generaTokenPass($user_id) {
    ...
    $stmt = $con->prepare("UPDATE usuarios SET token_password=?, password_request=1 WHERE id_usuario = ?");
...
function getPasswordRequest($id) {
    ...
    $stmt = $con->prepare("SELECT password_request FROM usuarios WHERE id_usuario = ?");
    $stmt->bind_param('i', $id);
    $stmt->execute();
...
function verificaTokenPass($user_id, $token) {
    ...
    $stmt = $con->prepare("SELECT activacion FROM usuarios WHERE id_usuario = ? AND token_password = ? AND password_request = 1 LIMIT 1");
...
function cambiaPassword($password, $user_id, $token) {
    ...
    $stmt = $con->prepare("UPDATE usuarios SET password = ?, token_password='', password_request=0 WHERE id_usuario = ? AND token_password = ?");
